...
for (const id of ids) {
  ...
  const competitionContext = buildCompetitionMotivationContext({
    match: { ...match, competitionStage },
    research: { competitionStage },
  });
  const motivationEvidence = competitionContext.evidenceLevel === "VERIFIED"
    ? verified(competitionContext.summary, "A")
    : {
        status: "NOT_PUBLISHED",
        evidenceGrade: "C",
        summary: `${competitionContext.summary} 缺失可核验字段：${competitionContext.missingEvidence.join("、") || "赛事规则与积分目标"}；本层只展示事实，不把未知动机写入概率。`,
        capturedAt,
