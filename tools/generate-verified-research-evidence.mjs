...
for (const id of ids) {
  ...
  const official = officialByLeague[match.league];
  ...
  const sources = [official, apiSource, verifiedRecent?.source].filter(Boolean);
  ...
  const evidence = {
    ...
    motivation: verified(`官方赛程确认本场为2026赛季${match.league}常规联赛，按90分钟结算，无加时和点球分支；未将未核实动机写入概率。`, "A"),
  ...
  console.log(JSON.stringify({ id, output: `/tmp/evidence-${id}.json`, teamRows: [homeRows.length, awayRows.length], movementSnapshots: movement.cleanSnapshots || 0 }));
}
