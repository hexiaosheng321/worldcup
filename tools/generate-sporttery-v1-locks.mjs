...
const rows = (oddsData.matches || [])
  ...
  .map((item) => {
    ...
    const unified = runUnifiedPrediction({
      match: {
      ...
      market: { normal: item.normal || {}, handicapOdds: item.handicapOdds || {}, scoreOdds: item.scoreOdds || [], totalGoalsOdds: item.totalGoalsOdds || [] },
      oddsHistory: oddsContext.row?.history || {},
      samples: externalSamples,
      research: researchTemplate(item),
    }, { lockType: "PRE_LOCK" });
    const unifiedScores = unified.finalDecision.scores || [];
    ...
    return {
      ...
      homeProb: unified.featureSet.probabilities.HOME,
      drawProb: unified.featureSet.probabilities.DRAW,
      awayProb: unified.featureSet.probabilities.AWAY,
      xg: `${unified.featureSet.xg.home}-${unified.featureSet.xg.away}`,
      poisson: unifiedScores.join(" / "),
      ...
      finalDecisionAction: `统一引擎PRE_LOCK：胜平负${unified.finalDecision.winDrawLose}；让球${unified.finalDecision.handicapPick}；总进球${unified.finalDecision.totalGoalsPick}；比分${unifiedScores.join(" / ")}。阻断项：${unified.gateResult.blockers.join("、") || "无"}。`,
      pick: unified.finalDecision.winDrawLose,
      handicapPick: unified.finalDecision.handicapPick,
      totalGoalsPick: unified.finalDecision.totalGoalsPick,
      mainScore: unifiedScores[0] || scoreA,
      counterScore: unifiedScores[1] || scoreB,
