...
function mergeCloudAutoPredictions(rows = []) {
  ...
  let changed = false;
  rows.forEach((item) => {
    if (hasOfficialWorldCupLock(item, findSportteryItemForPrediction(item))) return;
    ...
    if (nextManual || (oldAuto && (!Number.isFinite(oldTime) || (Number.isFinite(nextTime) && nextTime >= oldTime)))) {
...
async function loadCloudBootstrapData({ rerender = false } = {}) {
  ...
  if (mergeCloudAutoPredictions([
    ...cloudLockRowsToPredictions(payload.locks || []),
    ...(payload.autoPredictions || []),
  ])) {
    changed = true;
  }
  if (changed && rerender) rerenderOddsSurfaces();
