...
function mergeCloudAutoPredictions(rows = []) {
  ...
  rows.forEach((item) => {
    ...
    if (oldAuto && (!Number.isFinite(oldTime) || (Number.isFinite(nextTime) && nextTime >= oldTime))) {
  ...
  const openItems = (oddsData.matches || [])
    .map((item) => {
      ...
      return {
        ...
        status: score ? "已完赛" : isLive ? liveScoreStatusText(liveScore) || "进行中" : modelPred ? "已有推演" : "已开盘",
      };
    })
...
async function loadCloudBootstrapData({ rerender = false } = {}) {
  ...
  if (mergeCloudAutoPredictions(payload.autoPredictions || [])) {
    changed = true;
  }
