...
function renderHome() {
  ...
  grid.innerHTML = upcoming
    ...
    .map((match) => {
      ...
      return `
        <button type="button" class="home-match-card ${liveStatus.tone === "live" ? "is-live" : ""} ${liveStatus.tone === "pending-result" ? "pending-result" : ""} ${liveStatus.tone === "finished" ? "finished" : ""}" data-home-match-tone="${liveStatus.tone}" ${cardTarget} aria-label="查看${match.home}对${match.away}赛事详情">
